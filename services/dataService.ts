...
// En Render, configuraremos la variable de entorno REACT_APP_API_URL
const API_BASE_URL = (window as any).process?.env?.API_URL || 
...
class DataService {
  ...
  async login(usuario: string, clave: string): Promise<{ success: boolean; user?: any; error?: string }> {
    try {
      ...
      return { success: false, error: 'No se pudo contactar con el servidor en Render' };
    }
  }
  ...
  async getProductos(): Promise<Product[]> {
    try {
      ...
      return data.map((p: any) => ({
        id: p.CodPro.trim(),
        name: p.Nombre.trim(),
        category: p.Clinea?.toString() || '0',
        price: p.PventaMa || 0,
        stock: p.Stock || 0,
        unit: p.Unimed?.toString() || '1',
        image: p.ImagenUrl || `https://picsum.photos/seed/${p.CodPro}/400`,
      }));
    } catch { return []; }
  }
